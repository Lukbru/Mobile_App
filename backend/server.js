...
app.post('/register', async (req, res) => {
    try {
        ...
        const result = await users.insertOne({ name, email, password: hashedPassword, isAdmin: false });
...
app.post('/login', async (req, res) => {
    try {
        ...
        res.json({ token, user: { _id: user._id, name: user.name, email: user.email, isAdmin: user.isAdmin || false } });
    } catch (error) {
        console.log(error);
        res.status(500).json({ message: 'Server error' });
    }
